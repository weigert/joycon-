...
buttonData InputBuffer::get_button_data(JOY_PID PID) const{
	/* Button Data Struct */
	buttonData data;
	...
	if(PID == JOYCON_R_BT){
		/* Buttons */
		data.a = ((buf[3] & 0x08) >> 3 == 1);
		data.b = ((buf[3] & 0x04) >> 2 == 1);
		data.x = ((buf[3] & 0x02) >> 1 == 1);
		data.y = ((buf[3] & 0x01) >> 0 == 1);

		/* Triggers */
		data.rTrigger = (((buf[3] >> 4) & 0x01) >> 0 == 1);
		data.lTrigger = (((buf[3] >> 4) & 0x02) >> 1 == 1);
		data.kTrigger = (((buf[3] >> 4) & 0x04) >> 2 == 1);
		data.zTrigger = (((buf[3] >> 4) & 0x08) >> 3 == 1);

		/* Special Buttons */
		data.special = (((buf[4] >> 4) & 0x01) >> 0 == 1);
		data.joyclick = ((buf[4] & 0x04) >> 2 == 1);
		data.symbol  = ((buf[4] & 0x02) >> 1 == 1);

		/* Joystick Data! */
		data.xJoy = (double)((buf[10] >> 4) + ((buf[10] & 0x0f) << 4))/255 - 0.5;
		data.yJoy = (double)(buf[11])/255 - 0.5;
	}
	else if(PID == JOYCON_L_BT){
		/* Buttons */
		data.a = ((buf[5] & 0x08) >> 3 == 1);
		data.y = ((buf[5] & 0x04) >> 2 == 1);
		data.b = ((buf[5] & 0x02) >> 1 == 1);
		data.x = ((buf[5] & 0x01) >> 0 == 1);

		/* Triggers */
		data.rTrigger = (((buf[5] >> 4) & 0x01) >> 0 == 1);
		data.lTrigger = (((buf[5] >> 4) & 0x02) >> 1 == 1);
		data.kTrigger = (((buf[5] >> 4) & 0x04) >> 2 == 1);
		data.zTrigger = (((buf[5] >> 4) & 0x08) >> 3 == 1);

		/* Special Buttons */
		data.special = (((buf[4] >> 4) & 0x02) >> 1 == 1);
		data.joyclick = ((buf[4] & 0x08) >> 3 == 1);
		data.symbol  = ((buf[4] & 0x01) >> 0 == 1);

		/* Joystick Data! */
		data.xJoy = (double)((buf[7] >> 4) + ((buf[7] & 0x0f) << 4))/255 - 0.5;
		data.yJoy = (double)(buf[8])/255 - 0.5;
	}
	return data;
}
